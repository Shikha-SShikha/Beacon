...
const CHECKS: CheckDef[] = [
  {
    ...
    step: "01",
    name: "Noise filter",
    description: "Remove terms that are clearly not scientific entities",
    whatItCatches: "Single letters (V, A), lab brand names (BioRad, Invitrogen), generic words (cells, genes, RNA)",
    failureMode: "Term is too short, too generic, or a known brand/instrument name",
    flags: [],
    autoReject: true,
  },
  {
    ...
    step: "02",
    name: "Category check",
    description: "Verify that the assigned category matches the type of database where the entity was found",
    whatItCatches: "A biological process tagged as Disease, a chemical process tagged as Chemical, equipment tagged as Cell Line",
    failureMode: "Entity resolves to a database namespace that contradicts the assigned category",
  ...
  },
  {
    ...
    step: "03",
    name: "Database match",
    description: "Confirm the entity exists in a reference database and the matched entry describes the same thing",
    whatItCatches: "Entities with no database entry, or entries where the official name is completely different from the tagged term",
    failureMode: "No database match found, or matched entry label has very low similarity to the entity text",
  ...
  },
  {
    ...
    description: "Confirm the same term is tagged with the same category throughout the entire article",
    whatItCatches: "METTL3 tagged as PROTEIN in Methods but GENE in Results — same term, two different categories",
    failureMode: "Same entity text appears with different types in different sections of the same article",
    flags: ["inconsistent"],
  },
  {
    id: "placement",
    step: "05",
    name: "Context check",
    description: "Flag entities whose category is unusual for the section they appear in",
    whatItCatches: "A DRUG entity in an Introduction section with no clinical framing, a DISEASE entity isolated only in Methods",
    failureMode: "Entity type is statistically rare in the section where it was found",
  ...
  },
  {
    ...
    description: "An independent automated review re-reads each surviving entity in context and flags disagreements",
    whatItCatches: "Subtle misclassifications that rule-based checks miss — processes labelled as chemicals, ambiguous abbreviations",
    failureMode: "Second reviewer disagrees with the original category assignment",
